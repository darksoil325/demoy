...
function getRandomPath() {
    ...
    brave: {
        ...
        ":scheme": "https",
        ":path": getRandomPath() + "?" + generateRandomString(3) + "=" + generateRandomString(5, 10),
        ...
        "sec-fetch-user": "?1",
        "sec-ch-device-memory": `${[1, 2, 4, 8][Math.floor(Math.random() * 4)]}`,
"viewport-width": `${Math.floor(Math.random() * (1920 - 320 + 1)) + 320}`,
"downlink": `${(Math.random() * (10 - 0.5) + 0.5).toFixed(1)}`,
"rtt": `${[50, 100, 200, 300][Math.floor(Math.random() * 4)]}`,
...
const browser = Math.random() < 0.6 ? getRandomBrowser() : (Math.random() < 0.5 ? "mobile" : "chrome");
